...
var API_URL = ""; // Store API URL dynamically
var TOTAL_USERS = 0; // Store total users dynamically
var CONCURRENCY_LIMIT = 0; // Store concurrency limit dynamically
var REQUEST_TYPE = ""; // Store request type dynamically
const HEADERS = {}; // Store headers dynamically
const BODY = {}; // Store body dynamically

// Function to get user headers
...
// Function to get body from user input
const getBody = async () => {
  while (true) {
    console.log("Enter body in key:value format (e.g., name:ashik, age:12). Press Enter when done.");
    const bodyInput = await rl.question("Body (key:value) or Enter to skip: ");

    if (!bodyInput) break;

    const bodyArray = bodyInput.split(",").map((item) => item.trim());
    for (const item of bodyArray) {
      const [key, value] = item.split(":").map((str) => str.trim());

      if (!key || !value) {
        console.log("❌ Invalid body format. Use key:value (e.g., name:ashik).");
        continue;
      }
  ...
  try {
    ...
    switch (REQUEST_TYPE) {
      case "GET":
        ...
        break;
      case "POST":
        response = await axios.post(API_URL, JSON.stringify(BODY), { headers });
        break;
      case "PUT":
        response = await axios.put(API_URL, JSON.stringify(BODY), { headers });
        break;
      case "DELETE":
        response = await axios.delete(API_URL, { headers, data: JSON.stringify(BODY) });
        break;
      default:
